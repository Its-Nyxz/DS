...
<head>
    ...
    @php
        ...
        // Cek apakah ada background, jika tidak gunakan default
        $backgrounds = $company->backgrounds
            ->map(function ($background) {
                return asset('storage/company/' . $background->image_path);
            })
            ->toArray(); // Convert to array for JavaScript
        $initialBackground = $backgrounds[0] ?? 'default-background.jpg'; // Gambar default
    @endphp
    @if ($logoPath)
...
<body id="background-carousel"
            ...
            <nav class="flex items-center justify-end gap-4">
                @auth
                    <a href="{{ url('/dashboard') }}"
                        class="inline-block px-5 py-1.5 bg-white dark:text-[#19140035] border-[#19140035] hover:border-[#1915014a] border text-[#1b1b18] dark:border-[#3E3E3A] dark:hover:border-[#62605b] rounded-sm text-sm leading-normal">
                        Dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}"
                        class="inline-block px-5 py-1.5  bg-white dark:text-[#19140035] border-[#19140035] hover:border-[#1915014a] border text-[#1b1b18] dark:border-[#3E3E3A] dark:hover:border-[#62605b] rounded-sm text-xl leading-normal">
                        Log in
                    </a>
    ...
    <div
        ...
        <main class="flex max-w-sm w-full flex-col-reverse lg:max-w-4xl lg:flex-row">
            ...
            <div
                class="text-[13px] leading-[20px] flex-1 p-6 pb-12 lg:p-20 bg-white dark:text-[#0c0c0c] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-es-lg rounded-ee-lg lg:rounded-ss-lg lg:rounded-ee-none">
                <div class="text-center">
                    @if ($logoPath)
                        <img src="{{ $logoPath }}" alt="Logo {{ $company->name }}" class="h-16 mx-auto mb-4">
                    @endif
                    ...
                    <p class="text-lg mt-2 text-gray-700 dark:text-gray-400">
                        {{ $company->slogan ?? 'Slogan perusahaan Anda akan tampil di sini.' }}
                    </p>
                    ...
                    @if ($company->address)
                        @if ($company->address_link)
                            <p class="text-sm mt-4 text-gray-500">
                                📍 <a href="{{ $company->address_link }}" class="underline hover:text-blue-600"
                    ...
                    {{-- Informasi Kontak --}}
                    <div class="mt-6 space-y-2 text-sm text-gray-700 dark:text-gray-400">
                        @if ($company->phone)
                            <p>
            ...
            <div class="relative lg:-ms-px -mb-px lg:mb-0 rounded-t-lg lg:rounded-t-none lg:rounded-e-lg! w-full lg:w-[438px] shrink-0 overflow-hidden"
                x-data="{ active: 0, total: {{ $company->banners->count() }} }" x-init="setInterval(() => { active = (active + 1) % total }, 5000)">
                <div class="flex transition-transform duration-500 ease-in-out w-full h-full"
                    ...
                    @foreach ($company->banners as $banner)
                        <div class="w-full flex-shrink-0">
                            <img src="{{ asset('storage/company/' . $banner->image_path) }}"
                                alt="{{ $banner->title ?? 'Banner' }}" class="w-full h-full object-cover" />
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    @endif
    <script>
        // Ambil array background dari PHP
