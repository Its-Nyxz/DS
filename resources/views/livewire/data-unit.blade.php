<x-card title="Data Satuan">
    <div class="p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse($units as $unit)
                <div class="bg-white dark:bg-zinc-800 shadow rounded p-4 flex flex-col justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-300">Simbol: <strong>{{ $unit->symbol }}</strong>
                        </p>
                    </div>
                    <div class="flex items-center space-x-2 mt-4">
                        ...
                        <!-- Delete Button -->
                        <button type="button"
                            onclick="confirmAlert('Yakin ingin menghapus data ini?', 'Ya, hapus!', () => @this.call('delete', {{ $unit->id }}))"
            ...
            @empty
                <div class="col-span-full text-center text-gray-500">Tidak ada data satuan.</div>
            @endforelse
        </div>
        ...
        <div x-data="{ open: @entangle('isModalOpen') }">
            ...
            <div x-show="open"
                ...
                x-transition>
                <h2 class="text-xl font-semibold mb-4">{{ $unitId ? 'Edit' : 'Tambah' }} Satuan</h2>
                ...
                <div class="mb-4">
                    <label class="block mb-1">Simbol</label>
                    <input type="text" wire:model.defer="symbol" class="w-full border rounded px-3 py-2">
                    @error('symbol')
                        <small class="text-red-500">{{ $message }}</small>
                    @enderror
                </div>
